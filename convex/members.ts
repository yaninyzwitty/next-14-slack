...
export const current = query({
    ...
    handler: async (ctx, args) => {
        ...
        const member = await ctx.db.query('members').withIndex('by_workspaceId_and_userId', q => q.eq("workspaceId", args.workspaceId).eq("userId", userId)).unique()
...
export const getById = query({
    ...
    handler: async (ctx, args) => {
        ...
        if(!member) return null;
        const currentMember = await ctx.db.query('members').withIndex('by_workspaceId_and_userId', q => q.eq('workspaceId', member.workspaceId).eq('userId', userId)).unique()
...
export const update = mutation({
    ...
    async handler(ctx, args) {
        ...
        .withIndex('by_workspaceId_and_userId', q => q.eq('workspaceId', member.workspaceId)
...
export const remove = mutation({
    ...
    async handler(ctx, args) {
        ...
        const [messages, reactions, conversations
